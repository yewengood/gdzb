...
case $city_choice in
	...
	channel_key="广东移动"
        url_fofa=$(echo  '"udpxy" && country="CN" && region="Guangdong" && org="China Mobile Communications Group Co., Ltd."' | base64 |tr -d '\n')
        url_fofa="https://fofa.info/result?qbase64="$url_fofa
        ;;
    3)
        city="Guangdong_145"
        ...
        channel_key="广东联通"
        url_fofa=$(echo  '"udpxy" && country="CN" && region="Guangdong" && org="China Unicom IP network China169 Guangdong province"' | base64 |tr -d '\n')
        url_fofa="https://fofa.info/result?qbase64="$url_fofa
        ;;
