...
case $city_choice in
    1)
        ...
        channel_key="四川电信"
        url_fofa=$(echo  '"udpxy" && country="CN" && region="Sichuan"' | base64 |tr -d '\n')
    ...
    2)
        ...
        channel_key="北京联通"
        url_fofa=$(echo  '"udpxy" && country="CN" && region="Beijing"' | base64 |tr -d '\n')
        url_fofa="https://fofa.info/result?qbase64="$url_fofa
        ;;
...
cat txt/Sichuan.txt >>zubo_fofa.txt
echo "📡  北京联通,#genre#" >>zubo_fofa.txt
cat txt/Beijing.txt >>zubo_fofa.txt
